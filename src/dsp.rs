...
use crate::config::{AttenuationControl, OcclusionControl, PanningControl};
...
impl BiquadState {
    #[inline(always)]
    pub(crate) fn process_sample(&mut self, input: f32, coeffs: &BiquadCoefficients) -> f32 {
...
impl AttenuationFilter {
    ...
    pub(crate) fn process(&mut self, buffer: &mut BlockBuffer) {
    ...
    pub fn process(&mut self, buffer: &mut BlockBuffer) {
        ...
        for frame_chunk in buffer.frames_mut() {
            for (channel, sample) in frame_chunk.iter_mut().enumerate() {
                ...
                let is_unoccluded = curr_low >= 0.999 && curr_mid >= 0.999 && curr_high >= 0.999;
                if is_unoccluded {
                    *sample = input;
                } else {
                    let filtered = (s_low * curr_low) + (s_mid * curr_mid) + (s_high * curr_high);
                    let mix = ((1.0 - curr_low) + (1.0 - curr_mid) + (1.0 - curr_high)) / 3.0;
                    *sample = input + (filtered - input) * mix.clamp(0.0, 1.0);
                }
            }
...
impl PanningFilter {
    ...
    pub(crate) fn process_stereo(&mut self, buffer: &mut BlockBuffer) {
        let frames_count = buffer.frames_count();
        if frames_count == 0 {
            return;
        }
        ...
        for frame in buffer.frames_mut() {
            if frame.len() >= 2 {
                frame[0] = soft_clip(frame[0] * curr_left);
                frame[1] = soft_clip(frame[1] * curr_right);
            }
    ...
    pub(crate) fn process_mono_to_stereo(&mut self, input: &BlockBuffer, output: &mut BlockBuffer) {
        output.clear();
...
mod tests {
    ...
    fn test_occlusion_chain_unoccluded_bypass() {
        let control = Arc::new(OcclusionControl {
            gain_low: crate::config::AudioParam::new(1.0),
            gain_mid: crate::config::AudioParam::new(1.0),
            gain_high: crate::config::AudioParam::new(1.0),
            perceived_dir_x: crate::config::AudioParam::new(0.0),
            perceived_dir_y: crate::config::AudioParam::new(0.0),
            perceived_dir_z: crate::config::AudioParam::new(0.0),
        });
